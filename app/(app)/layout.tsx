...
export default function RootLayout({ children }: RootLayoutProps) {
  ...
  // Lógica de logout separada para que el onClick no se confunda
      ...
      `}>
        ...
        <div className="p-4 border-t border-gray-100 shrink-0">
          {user ? (
            ...
            <Link
              href="/login"
              className="flex items-center gap-4 w-full px-4 py-3 text-gray-400 hover:text-primary-blue transition-colors"
            >
              <LogOut size={22} className="shrink-0" />
              <span className={`text-xs font-black uppercase tracking-widest whitespace-nowrap transition-opacity duration-300 ${isExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                {t('menu.login')}
            ...
            <div className="mt-auto pt-6 border-t border-gray-100">
              {user ? (
                ...
                <Link
                  ...
                  className="w-full py-4 bg-primary-blue text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-blue-100 flex justify-center"
                >
                  {t('menu.login')}
                </Link>
              )}
